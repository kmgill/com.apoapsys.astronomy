package com.apoapsys.astronomy.perlin;

import com.apoapsys.astronomy.math.MathExt;


public class RidgedPerlin extends Perlin {
	
	private final PerlinNoiseModifier modifier = new RidgedPerlinModifier();
	
	public RidgedPerlin(double persistence, double numberOfOctaves, double resolution) {
		super(persistence, numberOfOctaves, resolution);
	}
	
	@Override
	public double noise(double x, double y) {
		return super.noise(x, y, modifier);
	}
	
	class RidgedPerlinModifier implements PerlinNoiseModifier {

		@Override
		public double onNoise(double x, double y, double noise,
				double frequency, double amplitude, double octaveNum) {
			noise = 1.0 - MathExt.abs(noise);
			return noise;
		}

		@Override
		public double onTotal(double total) {
			return total;
		}
		
	}
	
}
